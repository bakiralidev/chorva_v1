...
class Region(Base):
    ...
    def set_translation_field(self, lang: str, field: str, value: str) -> None:
        ...
        new_t = RegionTranslation(language=lang)
    ...
    def to_response(self, lang: str) -> dict:
        translated = self.translate(lang, ["name"])
        return {
            "id": self.id,
    ...
    def __str__(self) -> str:
        ...
        return self.name_uz or self.name_ru or f"Region #{self.id}"


class RegionTranslation(Base):
    __tablename__ = "region_translations"
    __table_args__ = (
        UniqueConstraint("region_id", "language", name="uq_region_lang"),
    )
    ...
    # Relationships
    region: Mapped[Region] = relationship(Region, back_populates="translations")
